...
async def get_fern_writer_install_link(
    ...
    org_id: str | None = Query(
        None,
        description="The organization ID to associate with this integration.",
    ),
) -> JSONResponse:
    try:
        ...
        async with async_session_maker() as session:
            ...
            LOGGER.info(
                f"[SCRIBE] Created new integration {integration_id} " f"for GitHub repo {github_repo} (org_id={org_id})"
            )
