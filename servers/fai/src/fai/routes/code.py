...
async def create_code_record(
    ...
    await sync_documents_to_query_index(domain, created_code_ids, get_code_index_name(), get_query_index_name())
    LOGGER.info(f"Indexed code {new_db_code.id} for domain: {domain}")
    return [CreateCodeRecordResponse(code_id=code_id) for code_id in created_code_ids]
...
async def batch_create_code_records(
    domain: str,
    body: list[CreateCodeRecordRequest] = Body(...),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(verify_token),
    __: None = Depends(ask_ai_enabled),
) -> list[CreateCodeRecordResponse]:
...
@fai_app.get(
    "/code/{domain}/{code_id}",
    response_model=GetCodeRecordResponse,
    openapi_extra={"x-fern-audiences": ["customers"], "security": [{"bearerAuth": []}]},
)
async def get_code_record_by_id(
    ...
    return GetCodeRecordResponse(document=code.to_api())


@fai_app.get(
    "/code/{domain}",
    response_model=GetCodeRecordsResponse,
    openapi_extra={"x-fern-audiences": ["customers"], "security": [{"bearerAuth": []}]},
)
...
@fai_app.delete(
    "/code/{domain}/delete",
    response_model=DeleteCodeRecordResponse,
    openapi_extra={"x-fern-audiences": ["customers"], "security": [{"bearerAuth": []}]},
)
async def delete_code_record_by_id(
    domain: str,
    body: DeleteCodeRecordRequest = Body(...),
    db: AsyncSession = Depends(get_db),
    _: None = Depends(verify_token),
    __: None = Depends(ask_ai_enabled),
) -> DeleteCodeRecordResponse:
    ...
    if db_code:
        ...
        await delete_documents_from_query_index(domain, [body.code_id], get_code_index_name(), get_query_index_name())
        LOGGER.info(f"Deleted code {body.code_id} for domain: {domain}")
        return DeleteCodeRecordResponse(success=True)
    return DeleteCodeRecordResponse(success=False)
