    ...
    for session_record in active_sessions:
        async with async_session_maker() as db_session:
            integration_result = await db_session.execute(
                select(ScribeIntegrationDb).where(ScribeIntegrationDb.integration_id == session_record.integration_id)
            )
            integration = integration_result.scalar_one_or_none()

            if not integration or not integration.slack_bot_token:
                LOGGER.warning(
                    f"[SCRIBE] Cannot resume session {session_record.devin_session_id}: "
                    "missing integration or bot token"
                )
                continue
