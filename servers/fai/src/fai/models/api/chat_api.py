...
class PostChatCompletionRequest(BaseModel):
    ...
    max_tokens: int | None = Field(
        ...
        le=1000,
        ge=100,
        description="The maximum number of tokens to generate. Defaults to 1000. "
        + "Note: setting a token count lower than 400 may result in incomplete responses. You can add a custom "
        + "system prompt to let the model know the maximum token count it can use.",
    )
