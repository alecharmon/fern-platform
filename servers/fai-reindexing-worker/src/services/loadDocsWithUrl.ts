...
export async function loadDocsWithUrl(payload: LoadDocsWithUrlPayload): Promise<LoadDocsWithUrlResponse> {
    ...
    if (!docs.ok) {
        throw new Error(`Failed to get docs for ${payload.domain}: ${JSON.stringify(docs.error)}`);
    }
    ...
    if (!res.ok) {
        throw new Error(`Failed to get org for ${payload.domain}: ${JSON.stringify(res.error)}`);
    }
