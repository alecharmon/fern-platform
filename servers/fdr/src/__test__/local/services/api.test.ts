...
const MOCK_REGISTER_API_DEFINITION: APIV1Write.ApiDefinition =
  createApiDefinition({
...
it("register api", async () => {
  ...
  console.log(
    `Registered empty definition. Received ${emptyDefinitionRegisterResponse.apiDefinitionId}`
  );
  // load empty definition
  const registeredEmptyDefinition = getAPIResponse(
    await fdr.api.v1.read.getApi(
      emptyDefinitionRegisterResponse.apiDefinitionId
    )
  );
  ...
  expect(registeredEmptyDefinition.rootPackage).toEqual(
    EMPTY_REGISTER_API_DEFINITION.rootPackage
  );
  ...
  const updatedDefinition = getAPIResponse(
    await fdr.api.v1.read.getApi(
      updatedDefinitionRegisterResponse.apiDefinitionId
    )
  );
  // assert definitions equal
  expect(JSON.stringify(updatedDefinition.types)).toEqual(
    JSON.stringify(MOCK_REGISTER_API_DEFINITION.types)
  );
...
const MOCK_REGISTER_API_LATEST_DEFINITION: FdrAPI.api.latest.ApiDefinition =
  createApiDefinitionLatest({
...
it("register api latest", async () => {
  ...
  // load empty definition
  const registeredEmptyDefinition = getAPIResponse(
    await fdr.api.latest.getApiLatest(
      emptyDefinitionRegisterResponse.apiDefinitionId
    )
  );
  ...
  // assert definitions are equal
  expect(JSON.stringify(registeredEmptyDefinition.types)).toEqual(
    JSON.stringify(emptyDefinition.types)
  );
  expect(JSON.stringify(registeredEmptyDefinition.subpackages)).toEqual(
    JSON.stringify(emptyDefinition.subpackages)
  );
  ...
  // assert definitions equal
  expect(JSON.stringify(updatedDefinition.types)).toEqual(
    JSON.stringify(MOCK_REGISTER_API_LATEST_DEFINITION.types)
  );
  expect(JSON.stringify(updatedDefinition.subpackages)).toEqual(
