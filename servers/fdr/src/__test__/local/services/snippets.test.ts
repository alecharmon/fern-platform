...
it("no snippets generated when dynamicIR is present with regular definition", async () => {
    ...
    expect(apiDefinition?.snippetsConfiguration?.typescriptSdk?.package).toEqual("acme");
    expect(apiDefinition?.snippetsConfiguration?.typescriptSdk?.version).toEqual("0.0.3");
});
