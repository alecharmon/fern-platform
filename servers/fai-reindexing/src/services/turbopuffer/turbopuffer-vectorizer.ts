    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (attempt < maxRetries - 1) {
                logger.info(
                    `Attempt ${attempt + 1}/${maxRetries} failed: ${errorMessage}. Retrying in ${Math.round(waitMs)}ms...`,
                    { attempt: attempt + 1, maxRetries, errorMessage, waitMs }
                );
    ...
    return async (chunks: string[]) => {
        ...
        async function sendBatch(isFinal = false): Promise<void> {
            ...
            try {
                ...
                logger.info(
                    `Batch ${totalBatches}${isFinal ? " (final)" : ""}: ${payload.length} chunks, ${batchTokens} tokens, ${batchDuration}ms`
                );
