...
import { mergePdfDocuments } from "./merge-pdf-documents";
 ...
 */
const HEADER_FOOTER_TEXT_COLOR = rgb(156 / 255, 163 / 255, 175 / 255);

const FILE_METADATA_CREATOR = "Fern";
const FILE_METADATA_PRODUCER = "Fern PDF Generator";
...
export interface DocsPdfGeneratorConfig {
    /**
     ...
     *
     * This should be the Fern admin token (`FERN_TOKEN` environment variable).
     */
    authToken?: string;
}
 ...
 */
const DEFAULT_DOCS_PDF_GENERATOR_CONFIG: DocsPdfGeneratorConfig = {
 ...
 */
export class DocsPdfGenerator {
    private readonly config: DocsPdfGeneratorConfig;
     ...
     */
    public constructor(config: Partial<DocsPdfGeneratorConfig> = {}) {
    ...
    private createLogger() {
        switch (this.config.logFormat) {
            case "json":
                return createConsoleJsonLogger({ component: "DocsPdfGenerator" });
            case "pretty":
                return createPrettyConsoleLogger({ component: "DocsPdfGenerator" });
            default:
    ...
    public async start() {
        ...
        try {
            (this as DocsPdfGenerator).browser = await chromium.launch({ headless: true });
    ...
    public async stop() {
        ...
        (this as DocsPdfGenerator).browser = null;
     ...
     */
    public assertStarted(): asserts this is DocsPdfGenerator & { browser: Browser } {
        if (this.browser === null) {
            throw new Error("Browser is not launched. Call start() first.");
        }
    }

    public assertNotStarted(): asserts this is DocsPdfGenerator & { browser: null } {
        if (this.browser !== null) {
