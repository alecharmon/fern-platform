    ...
    while (urlsToVisit.length > 0) {
        ...
        page.on("console", (msg) => {
            if (msg.type() === "error") {
                ...
                const isExpectedStatusError =
                    expectedStatus !== 200 && errorText.includes(`status of ${expectedStatus}`);
                ...
                if (expectedContentType) {
                    ...
                    if (!actualContentType.includes(expectedContentType)) {
                        ...
                        if (currentUrl.endsWith(".txt") || expectedContentType === "text/plain") {
                            ...
                            console.log(
                                `  ✅ Success (${content.length} bytes, status ${actualStatus}, content-type: ${actualContentType})`
                            );
                        } else {
