...
function toChangelogDb(
  ...
  return {
    ...
    urlSlug:
      ...
      (writeShape.title != null ? kebabCase(writeShape.title) : "changelog"),
    ...
    tabbed: (config) => {
      ...
      config.tabs?.forEach((tab) => {
        if (isNavigationTabLink(tab)) {
          ...
          if (tab.items) {
            toRet.push(
              ...tab.items.flatMap(getReferencedApiDefinitionIdFromItem)
            );
          }
        }
      });
  ...
  return {
    ...
    postman:
      writeShape.postman != null
        ? transformPublishedPostmanCollectionForReading(writeShape.postman)
        : undefined,
  };
}
  ...
  switch (writeShape.type) {
    case "dark":
      return {
        type: "dark",
        accentPrimary:
          writeShape.accentPrimary ?? DEFAULT_DARK_MODE_ACCENT_PRIMARY,
    ...
    case "light":
      return {
        type: "light",
        accentPrimary:
          writeShape.accentPrimary ?? DEFAULT_LIGHT_MODE_ACCENT_PRIMARY,
        logo: writeShape.logo ?? docsConfig.logoV2?.dark ?? docsConfig.logo,
        background:
          writeShape.background != null
            ? { type: "solid", ...writeShape.background }
    ...
    case "darkAndLight":
      return {
        ...
        light: {
          accentPrimary:
            writeShape.light.accentPrimary ?? DEFAULT_LIGHT_MODE_ACCENT_PRIMARY,
          logo:
          ...
          background:
            writeShape.light.background != null
              ...
              : { type: "gradient" },
          backgroundImage:
            writeShape.light.backgroundImage ?? docsConfig.backgroundImage,
        ...
        dark: {
          accentPrimary:
            writeShape.dark.accentPrimary ?? DEFAULT_DARK_MODE_ACCENT_PRIMARY,
          logo:
            writeShape.dark.logo ?? docsConfig.logoV2?.dark ?? docsConfig.logo,
          background:
            writeShape.dark.background != null
              ? { type: "solid", ...writeShape.dark.background }
              : { type: "gradient" },
          backgroundImage:
            writeShape.dark.backgroundImage ?? docsConfig.backgroundImage,
...
function transformPageNavigationItemForDb(
  ...
  return {
    ...
    icon: writeShape.icon,
    urlSlug:
      writeShape.urlSlugOverride ?? defaultSlug ?? kebabCase(writeShape.title),
    fullSlug: writeShape.fullSlug,
    hidden: writeShape.hidden ?? false,
  };
}
