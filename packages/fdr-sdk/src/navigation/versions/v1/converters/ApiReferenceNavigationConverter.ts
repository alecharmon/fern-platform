...
export class ApiReferenceNavigationConverter {
    ...
    #visitedGraphqlOperations = new Set<FernNavigation.V1.GraphQlOperationId>();
        ...
        return this.#idgen.with(this.apiSection.urlSlug, (id) => {
            ...
            const changelog =
                this.apiSection.changelog != null
                    ...
                    : undefined;
            const pointsTo = FernNavigation.V1.followRedirects(children) ?? changelog?.slug;
            return {
                id,
                type: "apiReference",
    ...
    private convertChildren(parentSlug: SlugGenerator): FernNavigation.V1.ApiPackageChild[] {
    ...
    private convertGrpcNode(
        ...
        return this.#idgen.with(grpcId, (id) => {
            return {
                id,
                type: "grpc",
                title: grpcEndpoint.name ?? stringifyEndpointPathParts(grpcEndpoint.path.parts),
                grpcId,
    ...
    private convertPackageToChildren(
        ...
        (package_.endpoints ?? []).forEach((endpoint) => {
            if (endpoint.protocol?.type === "grpc" && endpoint.protocol.methodType != null) {
                ...
                children.push(this.convertGrpcNode(grpcId, endpoint, endpoint.protocol.methodType, parentSlug));
        ...
        (package_.subpackages ?? []).forEach((subpackageId) => {
            ...
            const child = this.#idgen.with(subpackageId, (id): FernNavigation.V1.ApiPackageNode | undefined => {
                ...
                const pointsTo = FernNavigation.V1.followRedirects(subpackageChildren);
                return {
                    id,
                    type: "apiPackage",
        ...
        items.forEach((item) => {
            visitDiscriminatedUnion(item, "type")._visit({
                ...
                endpointId: (oldEndpointId) => {
                    ...
                    if (endpoint.protocol?.type === "grpc") {
                        const grpcId = ApiDefinitionHolder.createGrpcId(endpoint, targetSubpackageId);
                        children.push(
                            this.convertGrpcNode(grpcId, endpoint, endpoint.protocol.methodType ?? "UNARY", parentSlug)
                        );
                        this.#visitedGrpcs.add(grpcId);
                    } else {
                ...
                subpackage: ({ subpackageId, items, summaryPageId }) => {
                    ...
                    const overviewPageId = summaryPageId != null ? FernNavigation.V1.PageId(summaryPageId) : undefined;
                    const noindex = overviewPageId != null ? this.noindexMap[overviewPageId] : undefined;
