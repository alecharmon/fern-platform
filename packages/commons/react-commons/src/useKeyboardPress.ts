...
import type { Digit, UppercaseLetter } from "@fern-api/ui-core-utils";
...
export function useKeyboardPress(args: useKeyboardPress.Args): void {
    ...
    useEffect(() => {
        ...
        document.addEventListener("keydown", (e) => void handleSaveKeyPress(e), capture);

        return () => {
            document.removeEventListener("keydown", (e) => void handleSaveKeyPress(e), capture);
        };
