...
describe("getAuthState", () => {
    ...
    it("should handle generalized oauth2", async () => {
        ...
        if (!authStateBadToken.authed) {
            ...
            expect(authorizationUrl.pathname).toBe("/oauth/authorize");
            expect(authorizationUrl.searchParams.get("response_type")).toBe("code");
            expect(authorizationUrl.searchParams.get("client_id")).toBe(GENERAL_OAUTH_CONFIG.clientId);
            expect(authorizationUrl.searchParams.get("redirect_uri")).toBe(
    ...
    it("should handle generalized oauth2 with authorize query params", async () => {
        const GENERAL_OAUTH_CONFIG = {
            ...
            clientId: "test_client_id",
            clientSecret: "test_client_secret",
            auth_endpoint: "https://auth.example.com/oauth/authorize?audience=https%3A%2F%2Fdemo.com",
            token_endpoint: "https://auth.example.com/oauth/token",
            scope: ["read", "write"],
            issuer: "https://example.com"
        };
        ...
        if (!authStateBadToken.authed) {
            ...
            expect(authorizationUrl.origin).toBe("https://auth.example.com");
            expect(authorizationUrl.pathname).toBe("/oauth/authorize");
            expect(authorizationUrl.searchParams.get("audience")).toBe("https://demo.com");
            expect(authorizationUrl.searchParams.get("response_type")).toBe("code");
            expect(authorizationUrl.searchParams.get("client_id")).toBe(GENERAL_OAUTH_CONFIG.clientId);
            expect(authorizationUrl.searchParams.get("redirect_uri")).toBe(
                "https://docs.test.com/api/fern-docs/oauth2/callback"
