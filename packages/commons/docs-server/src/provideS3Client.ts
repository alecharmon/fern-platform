...
export const provideS3Client = once((): S3Client | undefined => {
    ...
    try {
        assertNonNullish(process.env.AWS_ACCESS_KEY_ID, "AWS_ACCESS_KEY_ID is not set");
        assertNonNullish(process.env.AWS_SECRET_ACCESS_KEY, "AWS_SECRET_ACCESS_KEY is not set");
        return new S3Client({
