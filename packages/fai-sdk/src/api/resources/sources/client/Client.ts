...
export class Sources {
    ...
    private async __indexGithubSourceRepos(
        ...
        const _response = await core.fetcher({
            url: core.url.join(
                ...
                `sources/github/${encodeURIComponent(domain)}/index`,
            ),
            method: "POST",
            headers: _headers,
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.FernAITimeoutError(
                    "Timeout exceeded when calling POST /sources/github/{domain}/index.",
                );
            case "unknown":
                throw new errors.FernAIError({
    ...
    /**
     * @param {string} domain
     * @param {FernAI.IndexingCallbackRequest} request
     * @param {Sources.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link FernAI.UnprocessableEntityError}
     *
    ...
    public indexingCallback(
        domain: string,
        request: FernAI.IndexingCallbackRequest,
        requestOptions?: Sources.RequestOptions,
    ): core.HttpResponsePromise<FernAI.IndexingCallbackResponse> {
        return core.HttpResponsePromise.fromPromise(this.__indexingCallback(domain, request, requestOptions));
    }

    private async __indexingCallback(
        domain: string,
        request: FernAI.IndexingCallbackRequest,
        requestOptions?: Sources.RequestOptions,
    ): Promise<core.WithRawResponse<FernAI.IndexingCallbackResponse>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({ Authorization: await this._getAuthorizationHeader() }),
            requestOptions?.headers,
        ...
        const _response = await core.fetcher({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.FernAIEnvironment.Production,
                `sources/github/${encodeURIComponent(domain)}/lambda/callback`,
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
        ...
        if (_response.ok) {
            return { data: _response.body as FernAI.IndexingCallbackResponse, rawResponse: _response.rawResponse };
        }
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.FernAITimeoutError(
                    "Timeout exceeded when calling POST /sources/github/{domain}/lambda/callback.",
                );
            case "unknown":
                throw new errors.FernAIError({
                    message: _response.error.errorMessage,
