...
async function fromReadableStream(stream: ReadableStream<Uint8Array>): Promise<string> {
...
describe("rest", () => {
    it("should proxy a basic rest request", async () => {
        ...
        const request = new IncomingRequest(`https://proxy.ferndocs.com/https://example.com/test`);
    ...
    it("should proxy with search params", async () => {
        ...
        const request = new IncomingRequest(`https://proxy.ferndocs.com/https://example.com/test?foo=bar&baz=qux`);
        const ctx = createExecutionContext();
        const response = await worker.fetch(request, env, ctx);
        expect(await response.text()).toMatchInlineSnapshot(`"Success!"`);
        expect(response.headers.get("Content-Type")).toBe("text/plain");
    ...
    it("should proxy with headers", async () => {
        ...
        const request2 = new IncomingRequest(`https://proxy.ferndocs.com/https://example.com/test`, {
            method: "POST",
