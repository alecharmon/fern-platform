...
> = ({ additionalJsxElements = [] } = {}) => {
    return (ast) => {
        ...
        visit(ast, (node) => {
            ...
            if (isMdxJsxElementHast(node) && node.name && additionalJsxElements.includes(node.name)) {
                const idAttribute = node.attributes
                    .filter(isMdxJsxAttribute)
                    .find((attribute) => attribute.name === "id");
                if (idAttribute != null) {
...
function getTitle(node: Hast.MdxJsxElement) {
    const title = node.attributes.filter(isMdxJsxAttribute).find((attribute) => attribute.name === "title");
    if (title) {
