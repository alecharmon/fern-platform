 ...
 */
export function sanitizeMdxExpression(content: string): [string, handled: boolean] {
    ...
    while (loops++ < 100) {
        if (loops >= 100) {
            console.error("Infinite Loop Detected: sanitizing acorn failed:", content);
    ...
    errors.forEach((e) => {
        if (!e.handled) {
            console.error(`Unhandled MDX sanitization error: ${String(e.error.message)}: ${e.affectedLine}`);
        }
    });
...
function handleUnexpectedEOF(content: string, _e: VFileMessage): [string, handled: boolean] {
    const stack: [number, "{" | "<"][] = [];
...
function handleSpreadExtra(content: string, e: VFileMessage): [string, handled: boolean] {
    let handled = false;
...
function escapeCurrentLine(content: string, e: VFileMessage): [string, handled: boolean] {
...
function handleEndTagMismatch(content: string, e: VFileMessage): [string, handled: boolean] {
    ...
    const [tagStartLine, tagStartColumn, tagEndLine, tagEndColumn] = tagLocationStr
        .split(/[:-]/g)
        .map((s) => parseInt(s));
    ...
    return [
        content.slice(0, start) +
            content.slice(start, end).replaceAll(/</g, "&lt;").replaceAll(/>/g, "&gt;") +
            content.slice(end),
        true
    ];
}
