...
async function fetchSitemapUrls(hostname: string, sitemapUrl: string, token: string): Promise<string[]> {
...
function calculateSiteMetrics(
    ...
    const timeToFirstByteValues = successfulTimes.map((t) => t.timings.phases.firstByte ?? 0).sort((a, b) => a - b);
    const contentDownloadValues = successfulTimes.map((t) => t.timings.phases.download ?? 0).sort((a, b) => a - b);
    const totalDurationValues = successfulTimes.map((t) => t.timings.phases.total ?? 0).sort((a, b) => a - b);

    const calculateAverage = (values: number[]) => values.reduce((sum, val) => sum + val, 0) / values.length || 0;
...
async function testSite(site: { hostname: string; sitemapUrl: string; token: string }): Promise<SiteMetrics> {
    console.time(`[${site.hostname}]`);
    const urls = await fetchSitemapUrls(site.hostname, site.sitemapUrl, site.token);
    ...
    for (let i = 0; i < allRequests.length; i += BATCH_SIZE) {
        ...
        console.timeLog(`[${site.hostname}]`, `Finished batch ${i / BATCH_SIZE + 1}`);
    }
