...
export async function GET(request: NextRequest, { params }: { params: Promise<BaseParams> }) {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<BaseParams> }) {
    const authed = await isAuthed(request);
    if (!authed) {
        return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
