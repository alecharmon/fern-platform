...
export async function GET(req: NextRequest): Promise<NextResponse> {
    ...
    if (typeof config?.secret !== "string") {
        return redirectWithLoginError(req, redirectLocation, "missing_secret", "Couldn't login, please try again");
    }
    ...
    cookieJar.set(
        "fern_token",
        fern_token,
        withSecureCookie(withDefaultProtocol(preferPreview(host, domain)), undefined, domain)
    );
