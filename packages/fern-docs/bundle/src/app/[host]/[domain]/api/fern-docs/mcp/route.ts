    ...
    async (server) => {
      server.tool(
        ...
        "Search the documentation for relevant information.",
        {
          query: z
            .string()
            .describe("The search query to run against the docs"),
        },
        async ({ query }) => {
          // Use host and domain from the closure
          try {
            console.log(
              `[MCP] SearchDocs called with query:`,
              query,
              `host:`,
              host,
              `domain:`,
            ...
            const algoliaSearchKey = await fetch(
              `http://${domain}/api/fern-docs/search/v2/key`
            );
            ...
            if (!res.ok) {
              console.error(
                `[MCP] SearchDocs fetch failed: ${res.status} ${res.statusText}`
              );
              return {
                content: [
                  {
            ...
            if (!res.body) {
              throw new Error(
                "[MCP] Upstream response has no body for SSE stream"
              );
            }
