...
function fromKeyValuePairs(keyValuePairs: { key: unknown; value: unknown }[]): unknown {
    ...
    return (
        <li key={idx} className="flex min-h-12 flex-row items-start gap-1 py-2">
            ...
            <div className="min-w-0 flex-1 shrink">
                <span className="text-(color:--grayscale-a11) text-xs">{"key"}</span>
            ...
            <div className="min-w-0 flex-1 shrink">
                <span className="text-(color:--grayscale-a11) text-xs">{"value"}</span>
                <PlaygroundTypeReferenceForm
                    id={`${id}[${idx}].value`}
                    shape={valueShape}
                    value={item.value}
