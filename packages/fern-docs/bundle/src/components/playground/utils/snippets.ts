...
export function convertToCustomSnippetPayload(
    ...
    const headers = Object.entries(formState.headers).map(([name, value]) => ({
        name,
        value
    }));
    ...
    if (!auth && authState.header) {
        headers.push(
            ...Object.entries(authState.header.headers).map(([name, value]) => ({
                name,
                value
            }))
        );
    ...
    return {
        ...
        headers: Object.entries(formState.headers).map(([name, value]) => ({
            name,
            value
        })),
