...
export const serializeFormStateBody = async ({
    ...
    if (protocol?.type === "openrpc") {
        ...
        return {
            type: "json",
            value: wrapOpenRPCRequest(body == null ? {} : body.value, protocol.methodName)
        };
    }
            ...
            for (const [key, value] of Object.entries(body.value)) {
                switch (value.type) {
                    ...
                    case "fileArray":
                        formDataValue[key] = {
                            type: "fileArray",
                            value: (await Promise.all(value.value.map((value) => serializeFile(value)))).filter(
                                isNonNullish
                            )
                        };
                        break;
                    case "json": {
                        ...
                        if (property?.exploded) {
                            ...
                            const arrayValue = Array.isArray(value.value) ? value.value : [value.value];
                            formDataValue[key] = {
                                type: "exploded",
                                value: arrayValue,
                                contentType
