...
import type { PlaygroundFormDataEntryValue, PlaygroundFormStateBody } from "../types";
...
export function formDataFieldIsRequired(field: FormDataField, types: Record<string, TypeDefinition>): boolean {
    switch (field.type) {
        case "property":
...
export function getEmptyValueForField(
    ...
    if (field.type === "property") {
        return {
            type: "json",
            value: getEmptyValueForType(unwrapReference(field.valueShape, types).shape, types)
        };
