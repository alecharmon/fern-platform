...
export function PlaygroundEndpointMultipartForm({
    ...
    const handleFormDataFileChange = useCallback(
        (key: string, files: readonly File[] | undefined) => {
            ...
            if (files == null || files.length === 0) {
                ...
                setFormDataEntry(
                    key,
                    type === "files" ? { type: "fileArray", value: validFiles } : { type: "file", value: validFiles[0] }
                );
            }
        },
    ...
    const handleFormDataJsonChange = useCallback(
        (key: string, value: unknown) => {
            setFormDataEntry(key, (oldValue) => {
                const newValue = typeof value === "function" ? value(oldValue?.value) : value;
    ...
    const shownFields = formData.fields.filter((field) => {
        return formDataFieldIsRequired(field, types) || !!formDataFormValue[field.key];
    });

    const hiddenFields = formData.fields.filter((field) => !shownFields.includes(field));
                                ...
                                (field): FernDropdown.Option => ({
                                    ...
                                    tooltip:
                                        field.description != null ? (
                                            ...
                                            <Markdown size="xs" mdx={field.description} useNextMdx={false} />
                                        ) : undefined
                                })
                            ),
