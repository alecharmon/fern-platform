...
export const PlaygroundEndpointSelectorContent = forwardRef<HTMLDivElement, PlaygroundEndpointSelectorContentProps>(
    ({ apiGroups, className, shallow, replace }, forwardedRef) => {
                ...
                <li key={apiGroup.id}>
                    {apiGroup.breadcrumb.length > 0 && (
                        <div className="flex h-[30px] items-center truncate px-3 py-1">
                            {apiGroup.breadcrumb.map((breadcrumb, idx) => (
                                <Fragment key={idx}>
                                    {idx > 0 && <Slash className="size-icon-sm text-(color:--grayscale-a9) mx-0.5" />}
                    ...
                    <ul className="relative z-0 list-none">
                        {apiLeafNodes.map((node) => {
                            const active = removeTrailingSlash(pathname) === getExplorerBasePath(node.slug);
                            return (
        ...
        return (
            <FernTooltipProvider>
                <div className={cn("relative flex size-full flex-col", className)} ref={forwardedRef}>
