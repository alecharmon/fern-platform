...
function BuiltWithFernWatcher({ component }: { component: React.RefObject<HTMLAnchorElement | null> }) {
    useEffect(() => {
...
export function HideBuiltWithFern({ children }: { children: React.ReactNode }) {
    return <HideBuiltWithFernContext.Provider value={true}>{children}</HideBuiltWithFernContext.Provider>;
}
