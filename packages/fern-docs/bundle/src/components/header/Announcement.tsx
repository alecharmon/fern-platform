...
export function Announcement({
    ...
    const isDismissed = useAnnouncementStore((state) => state.announcement === announcement);
                ...
                <script
                    ...
                    dangerouslySetInnerHTML={{
                        __html: `(${String((announcement: string) => {
                            const dismissed = localStorage.getItem("fern-announcement-dismissed");

                            if (dismissed && JSON.parse(dismissed)?.state?.announcement === announcement) {
                                (window.requestAnimationFrame ?? window.setTimeout)(() => {
                                    const announcement = document.getElementById("fern-announcement");
                                    if (announcement) {
                                        announcement.remove();
                                    }

                                    const headerHeight = document.getElementById("fern-header")?.clientHeight;
