...
async function withLaunchDarklyContext(
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch LaunchDarkly context: ${response.statusText}`);
        }
...
export async function withLaunchDarkly(
    ...
    node: FernNavigation.utils.Node,
    rawCookie?: string
): Promise<[LaunchDarklyInfo | undefined, (node: FernNavigation.WithFeatureFlags) => boolean]> {
    ...
    if (launchDarklyConfig) {
        ...
        return [
            ...
            launchDarklyConfig["sdk-key"] ? await createLdPredicate({ flags }) : createDefaultFeatureFlagPredicate()
        ];
    }
    return [undefined, createDefaultFeatureFlagPredicate()];
}

function createDefaultFeatureFlagPredicate(): (node: FernNavigation.WithFeatureFlags) => boolean {
    return (node) =>
