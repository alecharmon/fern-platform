...
function convertBodyToContent(
    ...
    switch (body.type) {
        ...
        case "stream": {
            ...
            if (body.shape) {
                streamSchema = convertTypeShapeToSchema(body.shape, context);
            } else {
...
function convertTypeShapeToSchema(shape: AnyShapeType, context: ConversionContext): SchemaObject {
    if (!shape) {
    ...
    switch (shape.type) {
        ...
        case "enum": {
            const enumSchema: SchemaObject = {
                type: "string",
                enum: shape.values.map((v: { value: string }) => v.value)
            };
            if (shape.default !== undefined) {
        ...
        case "literal":
            if (shape.value.type === "booleanLiteral") {
                return { type: "boolean", enum: [shape.value.value] } as SchemaObject;
            }
            return { type: "string", enum: [shape.value.value] } as SchemaObject;
