    ...
    return (ast: Hast.Root) => {
        visit(ast, (node, index, parent) => {
            ...
            if (node.name === "StepGroup") {
                ...
                if (node.children.some((child) => isMdxJsxElementHast(child) && child.name === "Step")) {
...
function migrateStepGroup(node: Hast.MdxJsxElement) {
    ...
    for (const child of node.children) {
        if (
            ...
            (isMdxJsxElementHast(child) && child.name?.toLocaleLowerCase() === headingRank)
        ) {
            resetCurrentStep();
            currentStep = child;
        } else {
...
function getAttributes(node: Hast.MdxJsxElement | Hast.Element): (MdxJsxAttribute | MdxJsxExpressionAttribute)[] {
    if (node.type === "element") {
        return Object.entries(node.properties).map(([key, value]) => unknownToMdxJsxAttribute(key, value));
    }
    return [...node.attributes];
}

function createStep(title: Hast.MdxJsxElement | Hast.Element): Hast.MdxJsxElement {
    const attributes = getAttributes(title);
    if (!attributes.some((attribute) => attribute.type === "mdxJsxAttribute" && attribute.name === "title")) {
