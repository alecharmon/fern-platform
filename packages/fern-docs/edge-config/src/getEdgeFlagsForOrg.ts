...
type EdgeConfigResponse = Record<OrgEdgeFlag, string[] | Record<string, unknown>>;

export async function getEdgeFlagsForOrg(orgName: string): Promise<OrgEdgeFlags> {
    if (isLocal()) {
    ...
    try {
        ...
        const bypassExtendedGithubAuth = checkOrgNameMatchesCustomers(orgName, config["bypass-extended-github-auth"]);
...
function checkOrgNameMatchesCustomers(
    ...
    if (Array.isArray(customers)) {
        return (
            customers.some((customer) => orgName.toLowerCase().includes(customer.toLowerCase())) ||
            customers.includes(orgName)
        );
    } else {
        return (
            Object.keys(customers).some((key) => orgName.toLowerCase().includes(key.toLowerCase())) ||
