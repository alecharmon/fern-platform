...
export function getExampleDisplayName(example: CodeExample): string | null | undefined {
    return example.name ?? example.exampleCall.name;
}

export function isAutoGeneratedExampleName(name: string | null | undefined): boolean {
    return name?.endsWith("_example") ?? false;
}

export function isUserDefinedExample(example: CodeExample): boolean {
    const displayName = getExampleDisplayName(example);
    return displayName != null && !isAutoGeneratedExampleName(displayName);
}
