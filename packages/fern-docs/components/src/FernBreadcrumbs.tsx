...
export function FernBreadcrumbs({ breadcrumb }: FernBreadcrumbsProps): ReactElement<any> | null {
            ...
            {filteredBreadcrumbs.map((breadcrumb, idx) => (
                <Fragment key={idx}>
                    ...
                    {breadcrumb.pointsTo != null ? (
                        <FernLink className="fern-breadcrumb-item" href={slugToHref(breadcrumb.pointsTo)} scroll={true}>
                            {breadcrumb.title}
                        </FernLink>
                    ) : (
