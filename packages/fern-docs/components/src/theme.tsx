...
export function FernThemeProvider({
    ...
    const forcedTheme = enableSystem ? undefined : hasLight ? "light" : hasDark ? "dark" : undefined;
    ...
    return (
        <NextThemeProvider
            ...
            attribute="class"
        >
            <ThemeColorStyle lightThemeColor={lightThemeColor} darkThemeColor={darkThemeColor} />
            {children}
        </NextThemeProvider>
    );
}

function ThemeColorStyle({ lightThemeColor, darkThemeColor }: { lightThemeColor?: string; darkThemeColor?: string }) {
    ...
    React.useEffect(() => {
        ...
        const prefersLight = document.getElementById(THEME_COLOR_PREFERENCE_LIGHT_ID);
