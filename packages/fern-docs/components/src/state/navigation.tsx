...
type RootNodeState = {
    ...
    dispatch: (action: SidebarAction, currentSidebarRootNodeId: FernNavigation.NodeId) => void;
};
...
export function createRootNodeStore(
    ...
    return create<RootNodeState>((set) => ({
        state: new Map(),
        dispatch: (action: SidebarAction, currentSidebarRootNodeId: FernNavigation.NodeId) =>
            set(({ state }) => ({
...
const RootNodeStoreContext = React.createContext<UseBoundStore<StoreApi<RootNodeState>>>(
    createRootNodeStore(new Map())
);

const currentSidebarRootNodeIdAtom = atom<FernNavigation.NodeId | undefined>(undefined);
...
const currentProductIdAtom = atom<FernNavigation.ProductId | undefined>(undefined);
...
export function useIsChildSelected(nodeId: FernNavigation.NodeId) {
    ...
    const state = useStore((s) => {
        ...
        const parentToChildrenMap = invertParentChildMap(expandedState.childToParentsMap);
...
function reduceExpandedNodes(prev: ExpandedNodesState, action: SidebarAction): ExpandedNodesState {
    ...
    if (action.type === "expand-all") {
        return {
            expandedNodes: new Set(invertParentChildMap(prev.childToParentsMap).keys()),
            implicitExpandedNodes: new Set(),
            collapsedNodes: new Set(),
    ...
    const isExpanded = prev.expandedNodes.has(action.nodeId) || prev.implicitExpandedNodes.has(action.nodeId);

    const actionType: "expand" | "collapse" | "expand-soft" =
        action.type === "toggle" ? (isExpanded ? "collapse" : "expand") : action.type;
