...
export const LDFeatureFlagProvider: FC<Props> = ({
    ...
    return (
        <LDProvider clientSideID={clientSideId} context={defaultContext} flags={defaultFlags} options={options}>
            <LDClientWindow />
            {contextEndpoint ? (
                <IdentifyWrapper contextEndpoint={contextEndpoint} defaultContext={defaultContext}>
                    {children}
                </IdentifyWrapper>
            ) : (
                children
