...
export const LDFeature: React.FC<FeatureProps> = ({ flag, fallbackValue = false, match = true, children }) => {
...
export const LDFeatures: React.FC<WithFeatureFlagsProps> = ({ featureFlags, children }) => {
    const flags = useFlags();
    ...
    const predicates = featureFlags.map((featureFlag) =>
        ldFlagPredicate(featureFlag.flag, featureFlag.fallbackValue, featureFlag.match)
    );
