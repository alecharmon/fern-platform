...
describe("buildDocsYmlWithUpdates", () => {
    const createNavData = (baseContent: string, updates: Record<string, DocsYmlUpdate> = {}): StoredNavigationData => ({
        clientPages: {},
        docsYmlState: {
            baseContent,
    ...
    const createUpdate = (sectionTitle: string | null, page: string, path: string): DocsYmlUpdate => ({
        sectionTitle,
        pageEntry: { page, path },
        createdAt: Date.now(),
        operation: "add"
    ...
    it("returns base content when no pending updates", () => {
        const result = buildDocsYmlWithUpdates(createNavData("navigation:\n  - page: test"));
    ...
    it("throws error when pending updates exist but no base content", () => {
        ...
        for (const [key, update] of Object.entries(updates)) {
            expect(() => buildDocsYmlWithUpdates(createNavData("", { [key]: update }))).toThrow(
