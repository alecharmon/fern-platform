        ...
        tokens: (lines) => {
            ...
            return lines.map((tokens) => {
                ...
                if (templateMatches.length > 0) {
                    const validTemplates = templateMatches.filter((match) => templateVariables.has(match.variableName));
                ...
                while (i < tokens.length) {
                    ...
                    if (containsStart(tokens[i]?.content)) {
                        ...
                        if (openBraces === "{{" && closeBraces === "}}" && variableName.length > 0) {
                            ...
                            while (j < tokens.length && (openBraces !== "{{" || closeBraces !== "}}")) {
...
function processTokensWithTemplates(
    ...
    for (const template of templates) {
        ...
        if (startTokenIndex === endTokenIndex) {
            ...
            result = [...result.slice(0, startTokenIndex), ...newTokens, ...result.slice(startTokenIndex + 1)];
        } else {
            ...
            result = [...result.slice(0, startTokenIndex), ...newTokens, ...result.slice(endTokenIndex + 1)];
        }
    }
