...
function CommandUxProvider(props: {
    ...
    const value = useMemo(() => ({ inputError, setInputError, setInputRef }), [inputError, setInputError, setInputRef]);
    return <CommandUxContext.Provider value={value}>{props.children}</CommandUxContext.Provider>;
}
