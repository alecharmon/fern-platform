...
type InputProps = Omit<ComponentPropsWithoutRef<"input">, "value" | "onChange" | "type"> & {
    asChild?: boolean;
    /**
...
const useTriggerSelection = (): (() => void) => useContext(TriggerSelectionContext);
...
const Root = forwardRef<HTMLDivElement, CommandProps>((props, forwardedRef) => {
    ...
    function selectFirstItem() {
        ...
        const fallbackItem = getValidItems().find((item) => item.getAttribute("aria-disabled") !== "true");
...
const Item = forwardRef<HTMLDivElement, ItemProps>((props, forwardedRef) => {
    ...
    const render = useCmdk((state) =>
        forceMount ? true : !context.filter() ? true : !state.search ? true : (state.filtered.items.get(id) ?? 0 > 0)
    );
    ...
    return (
        <RadixDialog.Root open={open} onOpenChange={onOpenChange}>
            <RadixDialog.Portal container={container}>
                <RadixDialog.Overlay data-cmdk-overlay="" className={overlayClassName} />
                <RadixDialog.Content aria-label={props.label} data-cmdk-dialog="" className={contentClassName}>
...
function SlottableWithNestedChildren(
    ...
    if (asChild && isValidElement<{ children: ReactNode }>(children)) {
        return cloneElement(renderChildren(children), { ref: (children as any).ref }, render(children.props.children));
    }
