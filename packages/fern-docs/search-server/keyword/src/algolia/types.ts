...
export const SEARCHABLE_ATTRIBUTES = [
    ...
    "unordered(keywords)",
    // hierarchy (h1 -> h6)
    "unordered(hierarchy.h0)",
    "unordered(hierarchy.h1.title)",
    "unordered(hierarchy.h2.title)",
    "unordered(hierarchy.h3.title)",
    "unordered(hierarchy.h4.title)",
    "unordered(hierarchy.h5.title)",
    "unordered(hierarchy.h6.title)",
    "unordered(endpoint_path)",
    "unordered(endpoint_path_alternates)",
    "unordered(parameter_name)",

    "unordered(availability)",
    "unordered(api_type)",
    "unordered(method)",
    "unordered(section_type)",
    "unordered(subsection_type)",
    "unordered(response_type)",
    "unordered(status_code)",
    "unordered(parameter_type)",

    "unordered(breadcrumb)",
...
const EndpointBaseRecordSchema = BaseRecordSchema.extend({
    ...
    response_type: z.enum(["stream", "file", "json", "binary"]).optional(),
