...
function createTypeDefinitionSlots(
    ...
    return Object.fromEntries(
        Object.entries(types).flatMap(([id, type]) => {
            const variants = createPropertyAccessTypeVariants(id, type, types, componentProps);
            return [
                [id, variants.default],
...
function createPropertyAccessTypeVariants(
    ...
    return {
        default: <TypeReferenceDefinitions shape={type.shape} types={types} {...componentProps} />,
        request: <TypeReferenceDefinitions shape={type.shape} types={types} location="request" {...componentProps} />,
        response: <TypeReferenceDefinitions shape={type.shape} types={types} location="response" {...componentProps} />
    };
}
