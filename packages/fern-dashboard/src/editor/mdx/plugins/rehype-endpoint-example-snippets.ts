...
// Local utility function to extract method and path from endpoint string
function extractMethodAndPath(endpoint: string): { method: HttpMethod; path: string } | undefined {
    const [maybeMethod, path] = endpoint.trim().split(" ");

    // parse method into HttpMethod
 ...
 */
export const rehypeEndpointExampleSnippets: Unified.Plugin<[{ loader: DocsLoader }?], Hast.Root> = (opts) => {
    if (!opts) {
        return;
    }
