...
function Scene({ colors, speed }: { colors: GradientStop[]; speed: number }): React.JSX.Element {
    const { gl } = useThree();
    ...
    useFrame(({ clock }) => {
        ...
        planeRef.current.material.uniforms.uGradientStops.value.map((stop: { color: THREE.Vector4 }, index: number) => {
            stop.color.lerp(
                new THREE.Vector4(
                    new THREE.Color(targetColorsRef.current[index]?.color).convertLinearToSRGB().r,
                    new THREE.Color(targetColorsRef.current[index]?.color).convertLinearToSRGB().g,
                    new THREE.Color(targetColorsRef.current[index]?.color).convertLinearToSRGB().b,
