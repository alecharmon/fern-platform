...
const Item = (props: { item: SuggestionItem; isSelected: boolean; onSelect: () => void }) => {
    ...
    React.useEffect(() => {
        const selector = document.querySelector('[data-selector="tiptap-slash-dropdown-menu"]');
        if (!itemRef.current || !isSelected || !selector) return;

        const overflow = getElementOverflowPosition(itemRef.current, selector as HTMLElement);
...
const List = ({ items, selectedIndex, onSelect, config }: SuggestionMenuRenderProps & { config?: SlashMenuConfig }) => {
    const renderedItems = React.useMemo(() => {
        ...
        const groups: Record<string, { items: SuggestionItem[]; indices: number[] }> = {};
        ...
        Object.entries(groups).forEach(([groupLabel, groupData], groupIndex) => {
            if (groupIndex > 0) {
                rendered.push(<Separator key={`separator-${groupIndex}`} orientation="horizontal" />);
            }
