"use client";

import { useRouter } from "@bprogress/next/app";
import { Loader2 } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import { Button } from "@/components/ui/button";

interface AvailableOrg {
    orgName: string;
    displayName: string;
}

interface PostmanTeamSelectorProps {
    nextHref: string;
    postmanTeamId?: string;
    postmanCollectionId?: string;
}

function FernIcon({ className }: { className?: string }) {
    return (
        <svg viewBox="0 0 166 164" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
            <path
                d="M149.383 80.2222C138.594 71.101 122.341 67.4445 107.936 78.0925C107.273 78.5747 106.449 77.751 106.952 77.1081C110.367 72.7082 114.325 67.9668 117.519 63.2053C120.774 58.3233 125.636 54.8275 131.241 53.1198C161.076 44.079 152.116 0 152.116 0C152.116 0 106.027 2.97342 111.713 42.7329C112.657 49.3829 110.889 56.1535 106.731 61.4374C101.628 67.8865 95.7008 74.0543 91.4014 78.5144C90.4973 79.4386 88.9705 78.5546 89.3321 77.309C93.4909 63.3058 96.5246 41.648 82.1195 27.685L61.848 10.849L57.9504 15.9922C46.3581 31.2812 49.7534 52.8385 65.0625 64.4108C73.8422 71.0407 77.8201 78.2533 77.1973 86.169C76.8156 90.9104 74.6659 95.3505 71.4514 98.8663C65.4041 105.496 59.7586 112.608 55.3989 120.846C54.7962 121.991 53.0483 121.549 53.1086 120.243C53.7314 106.641 52.4255 75.983 29.5221 65.0336L3.88635 55.1289L1.89737 61.0556C-4.55174 80.182 5.99588 100.614 25.1021 107.104C41.7171 112.749 47.6439 123.457 43.6458 139.51C43.465 140.092 40.572 156.627 40.9738 163.96H59.3969C60.0198 152.589 71.9536 145.115 82.3003 149.756C85.2135 151.062 88.207 152.93 91.2809 155.341C107.755 168.32 132.025 165.246 144.983 148.752L148.68 144.05L125.375 127.315C109.383 114.738 88.0463 120.424 72.255 131.192C70.929 132.096 69.2414 130.65 69.9847 129.203C89.0709 91.7542 113.883 91.8346 123.607 100.152C135.4 110.238 153.261 108.429 163.266 96.5961L166.139 93.2007L149.363 80.2222H149.383Z"
                className="fill-fern"
            />
        </svg>
    );
}

export function PostmanTeamSelector({ nextHref, postmanTeamId, postmanCollectionId }: PostmanTeamSelectorProps) {
    const router = useRouter();
    const [orgs, setOrgs] = useState<AvailableOrg[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [navigatingOrgId, setNavigatingOrgId] = useState<string | null>(null);

    useEffect(() => {
        async function fetchAvailableOrgs() {
            try {
                const response = await fetch("/api/organization/available-for-postman");
                if (!response.ok) {
                    throw new Error("Failed to fetch organizations");
                }
                const data: AvailableOrg[] = await response.json();
                setOrgs(data);
            } catch (err) {
                setError(err instanceof Error ? err.message : "Failed to load organizations");
            } finally {
                setLoading(false);
            }
        }
        void fetchAvailableOrgs();
    }, []);

    const handleContinue = useCallback(
        async (orgId: string) => {
            setNavigatingOrgId(orgId);
            setError(null);

            try {
                // Join the org first (adds user to Venus + Auth0)
                const joinResponse = await fetch("/api/organization/join-postman-org", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ orgId, postmanTeamId })
                });

                if (!joinResponse.ok) {
                    const data = await joinResponse.json();
                    throw new Error(data.error ?? "Failed to join organization");
                }

                // Navigate to the org
                const destination = nextHref.includes(":orgId") ? nextHref.replace(/:orgId/g, orgId) : nextHref;
                const params = new URLSearchParams();
                if (postmanCollectionId) {
                    params.set("collection-id", postmanCollectionId);
                }
                if (postmanTeamId) {
                    params.set("postman-team-id", postmanTeamId);
                }
                const queryString = params.toString();
                router.push(queryString ? `${destination}?${queryString}` : destination);
            } catch (err) {
                setError(err instanceof Error ? err.message : "Failed to join organization");
                setNavigatingOrgId(null);
            }
        },
        [nextHref, postmanCollectionId, postmanTeamId, router]
    );

    if (loading) {
        return (
            <div className="flex items-center gap-2 py-4 text-sm text-muted-foreground">
                <Loader2 className="size-4 animate-spin" />
                Loading organizations...
            </div>
        );
    }

    if (error) {
        return <p className="text-sm text-destructive py-2">{error}</p>;
    }

    if (orgs.length === 0) {
        return <p className="text-sm text-muted-foreground py-2">No existing organizations found.</p>;
    }

    return (
        <div className="flex flex-col gap-5">
            {orgs.map((org) => (
                <div key={org.orgName} className="flex items-center gap-4">
                    <div className="flex size-10 shrink-0 items-center justify-center rounded-md border border-border bg-muted">
                        <FernIcon className="size-5" />
                    </div>
                    <span className="flex-1 truncate text-sm font-semibold">{org.displayName}</span>
                    <Button
                        type="button"
                        variant="outline"
                        size="sm"
                        onClick={() => handleContinue(org.orgName)}
                        loading={navigatingOrgId === org.orgName}
                        disabled={navigatingOrgId != null}
                    >
                        Connect
                    </Button>
                </div>
            ))}
        </div>
    );
}
