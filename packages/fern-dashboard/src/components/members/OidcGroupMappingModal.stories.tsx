...
import type { ExistingGroupMapping, OidcGroupMappingFormData } from "./OidcGroupMappingModal";
import { OidcGroupMappingModal } from "./OidcGroupMappingModal";
...
const ORG_LEVEL_MAPPINGS: ExistingGroupMapping[] = [{ groupId: "engineering", mappingType: "org_role", role: "admin" }];

const RESOURCE_LEVEL_MAPPINGS: ExistingGroupMapping[] = [
    { groupId: "design", mappingType: "resource_role", role: "editor", resourceId: "docs-public" },
    { groupId: "design", mappingType: "resource_role", role: "viewer", resourceId: "docs-api" }
];
...
export const WithOrgLevelMappings: Story = {
    name: "Existing org-level mapping",
    render: () => (
        <StatefulWrapper
            resources={PUBLIC_DOCS}
            existingGroupNames={SAMPLE_GROUP_NAMES}
            existingMappings={ORG_LEVEL_MAPPINGS}
            onSave={fn().mockName("onSave")}
        />
    )
};

export const WithResourceLevelMappings: Story = {
    name: "Existing resource-level mapping",
    render: () => (
        <StatefulWrapper
            resources={PUBLIC_DOCS}
            existingGroupNames={SAMPLE_GROUP_NAMES}
            existingMappings={RESOURCE_LEVEL_MAPPINGS}
            onSave={fn().mockName("onSave")}
        />
    )
};
