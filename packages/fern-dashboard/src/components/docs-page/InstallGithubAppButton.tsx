    ...
    const startBackgroundPoller = useCallback(() => {
        ...
        intervalRef.current = setInterval(() => {
            if (startedPollingAt.current == null) {
                console.warn("[startBackgroundPoller] startedPollingAt.current is null");
                cleanUp();
                return;
            }
            ...
            if (Date.now() - startedPollingAt.current > MAX_BACKGROUND_POLLING_TIME) {
                console.warn("[startBackgroundPoller] MAX_BACKGROUND_POLLING_TIME has been reached. Stopping poller.");
                cleanUp();
                return;
            }
