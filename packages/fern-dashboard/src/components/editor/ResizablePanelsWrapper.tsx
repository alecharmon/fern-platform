...
export function ResizablePanelsWrapper({ left, right, configPanel }: ResizablePanelsWrapperProps) {
    const { panelOpen } = useDevMode();
    const { isThemingPanelOpen } = useThemingPanel();
    const leftPanelRef = useRef<HTMLDivElement>(null);

    const isRightPanelOpen = panelOpen || isThemingPanelOpen;
    ...
    return (
        <PanelGroup direction="horizontal" className="h-full">
            ...
            <PanelResizeHandle
                className={cn("group relative w-4 bg-transparent !cursor-col-resize", !isRightPanelOpen && "hidden")}
            >
                <div className="pointer-events-none absolute inset-y-0 left-1 flex flex-col items-center justify-center">
            ...
            <Panel defaultSize={30} minSize={15} maxSize={70} className={isRightPanelOpen ? "" : "hidden"}>
                {isThemingPanelOpen ? configPanel : right}
            </Panel>
        </PanelGroup>
    );
}
