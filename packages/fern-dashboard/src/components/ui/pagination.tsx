...
const PaginationContent = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(
    ({ className, ...props }, ref) => (
        <ul ref={ref} className={cn("flex w-full flex-row items-center justify-center gap-1", className)} {...props} />
    )
);
...
const PaginationLink = ({ className, isActive, size = "icon", ...props }: PaginationLinkProps) => (
    <Button
        ...
        size={size}
        className={cn("h-8 w-8", isActive && "bg-primary text-primary-foreground hover:bg-primary/90", className)}
        {...props}
    />
);
...
const __generatePageNumbers = (currentPage: number, totalPages: number, maxVisiblePages: number) => {
    ...
    return (
        ...
        >
            <PaginationContent>
                ...
                {__generatePageNumbers(currentPage, totalPages, maxVisiblePages).map((page, index) => (
