...
export function CustomDomainCard({ docsUrl, orgName, domainInfo, allDomains = [] }: CustomDomainCardProps) {
    ...
    // Show verified domain - either fully configured (green) or needs setup (amber)
    if (domainInfo?.status === "VERIFIED") {
        ...
        return (
            <div className="flex flex-col items-center space-y-4">
                ...
                {/* Subpath domains: show reverse proxy instructions */}
                {isSubpathDomain && (
                    <div className="w-full rounded-md border border-border bg-muted/30 p-4">
                        ...
                        <p className="mt-3 text-xs text-muted-foreground">
                            Forward all requests from <code className="rounded bg-muted px-1">{domainInfo.domain}</code>{" "}
                            to the proxy target with the header above.
                        </p>
                    </div>
                )}

                {/* Regular domains: show DNS records */}
                {!isSubpathDomain && !isFullyConfigured && (
                    <div className="w-full rounded-md border border-border bg-muted/30 p-4">
                        <div className="mb-4 flex items-center justify-between">
                            ...
                            >
                                ...
                                <span className="ml-1">Check DNS</span>
                            </Button>
                        </div>
                        {loadingDns ? (
