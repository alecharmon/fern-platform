...
export const TIPTAP_COLLAB_DOC_PREFIX = process.env.NEXT_PUBLIC_TIPTAP_COLLAB_DOC_PREFIX || "";
export const TIPTAP_COLLAB_APP_ID = process.env.NEXT_PUBLIC_TIPTAP_COLLAB_APP_ID || "";
export const TIPTAP_COLLAB_TOKEN = process.env.NEXT_PUBLIC_TIPTAP_COLLAB_TOKEN || "";
...
export const removeEmptyParagraphs = (content: JSONContent) => ({
    ...
    content: content.content?.filter(
        (node) =>
            node.type !== "paragraph" || node.content?.some((child) => child.text?.trim() || child.type !== "text")
    )
});
 ...
 */
export function getElementOverflowPosition(targetElement: Element, containerElement: HTMLElement): OverflowPosition {
