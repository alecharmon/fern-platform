...
[(buildwithfern.com)](https://www.buildwithfern.com)`;
        ...
        // Persist the PR URL to the database so it survives modal/session reopens
        if (prResult.prUrl) {
            try {
                const verification = await getVerificationByDocsUrl(docsUrl);
                if (verification) {
                    await updatePrUrl(verification.id, prResult.prUrl);
                }
            } catch (err) {
                // Non-critical: log but don't fail the PR creation
                console.error("Failed to persist PR URL to database:", err);
            }
        }
