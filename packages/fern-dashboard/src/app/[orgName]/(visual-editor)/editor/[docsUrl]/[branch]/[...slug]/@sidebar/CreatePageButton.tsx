...
import { NavigationContext } from "@fern-docs/components/navigation/types";
...
export function CreatePageButton({ root, navigationContext }: CreatePageButtonProps) {
    const { prStatus } = useGitPrInfo();
    const isEditingDisabled = useEditingDisabled();

    // Check if there are sections available for creating pages
    const hasNoSections = !root?.children || getAllSections(root.children, [], root.id).length === 0;
    ...
    return (
        !hasNoSections && (
            ...
            >
                <CreateClientPage root={root} disabled={isEditingDisabled} navigationContext={navigationContext}>
                    <Button
