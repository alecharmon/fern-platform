...
export function CreateClientPage({ children, root, disabled = false, navigationContext }: CreateClientPageProps) {
    const [isPopoverOpen, setIsPopoverOpen] = useState(false);
    const [pageTitle, setPageTitle] = useState("");
    const [pageSlug, setPageSlug] = useState("");
    const [selectedSection, setSelectedSection] = useState<SectionWithHierarchy | null>(null);
    ...
    const allSections = useMemo(() => (root?.children ? getAllSections(root.children, [], root.id) : []), [root]);
    ...
    const validateForm = useCallback(() => {
        ...
        const clientDuplicate = existingClientPages.some((page) => page.slug === finalSlug);

        // Also check existing server pages in the selected section
        const existingServerPages = selectedSection.children?.filter((child) => child.type === "page") || [];
        const serverDuplicate = existingServerPages.some((page) => page.slug === finalSlug);
    ...
    const updateDocsYmlBaseContent = useCallback(async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to get docs.yml: ${response.status} ${response.statusText} - ${errorText}`);
            }
    ...
    const handleCreatePage = useCallback(async () => {
        ...
        try {
            ...
            const fullSlug = selectedSection.slug ? `docs/pages/${selectedSection.slug}/${slug}` : slug;
                                        ...
                                        return (
                                            <SelectItem key={section.id} value={section.id}>
                                                <div className="flex items-center">
                                                    ...
                                                    <span
                                                        className={
                                                            section.isUnnamed ? "text-muted-foreground" : undefined
                                                        }
                                                    >
