...
export class AnalyticsService {
  ...
  constructor(
    config: AnalyticsConfig & { projectId?: string; apiUrl?: string }
  ) {
    this.config = {
    ...
    const projectId =
      config.projectId || process.env.POSTHOG_ANALYTICS_PROJECT_ID;
    if (!projectId) {
      throw new Error(
        "POSTHOG_ANALYTICS_PROJECT_ID environment variable is required"
      );
    }
  ...
  async getPageViewsTimeSeries(
    ...
    if (
      groupBy &&
      results.length > 0 &&
      options.dateRange?.type === "last_n_days"
    ) {
      const dateRange = options.dateRange;
      if (dateRange.type === "last_n_days") {
        const requestedStartDate = new Date();
        requestedStartDate.setDate(
          requestedStartDate.getDate() - dateRange.days
        );
        const requestedStartDateString = requestedStartDate
          .toISOString()
          .split("T")[0];
  ...
  private buildDateAndFilterClause(options: { dateRange?: DateRangeOptions }): {
    ...
    if (dateRange.type === "last_n_days") {
      ...
      const endDate =
        "toStartOfWeek(now()) + interval 1 week - interval 1 second";
      ...
      const endDate =
        "toStartOfMonth(now()) + interval 1 month - interval 1 second";
   ...
   */
  private getQueryNameSuffix(options: {
    dateRange?: DateRangeOptions;
  }): string {
    const dateRange = options.dateRange || { type: "last_n_days", days: 7 };
