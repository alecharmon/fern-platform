...
function printMetrics(
  ...
  console.log(
    colors.bright + `📊 Analytics for ${colors.cyan}${url}${colors.reset}`
  );
...
async function main() {
  ...
  try {
    ...
    const baseUrl = await rl.question(
      colors.bright +
        "🌐 Enter the base URL to analyze (e.g., buildwithfern.com): " +
        colors.reset
    );
    ...
    const dateTypeChoice = await rl.question(
    ...
    const defaultValue =
      dateRangeType === "days" ? 7 : dateRangeType === "weeks" ? 4 : 6;
    ...
    if (showTimeSeries.toLowerCase() === "y") {
      ...
      timeSeries.forEach(({ date, value }) => {
        const barLength = Math.floor(
          (value / Math.max(...timeSeries.map((d) => d.value))) * 20
        );
