...
import { withSkewProtection } from "../utils/withSkewProtection";
...
export function useApiRouteSWR<T>(
    ...
    { disabled, request, validate, preload: shouldPreload, ...options }: Options<T> = {}
): SWRResponse<T> {
    const key = useApiRoute(route);
    if (!disabled && shouldPreload && (request?.method == null || request?.method === "GET")) {
